...
int odeRaycastGeom(float px, float py, float pz, float dx, float dy, float dz, float length, OdeVector out_pos, float & out_depth, unsigned long collideBits, unsigned long categoryBits)
{
	OdeThreadContext & c = contexts[s_getIdx];
	...
	if(nContacts > 0)
	{
		...
		for (GeomContainer::const_iterator it = c.geoms.begin(); it != c.geoms.end(); ++it)
		{
			if(it->second == closest->g2)
			{
				dGeomDestroy(ray);
				return it->first;
			}
